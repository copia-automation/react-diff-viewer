...
import getLinesToRender, { SkippedLineProps, RenderSkippedLineProps, LineInformationProps, ReactDiffViewerRenderProps, LineNumberPrefix } from './getLinesToRender';
			...
			<React.Fragment>
				...
				{!renderProps.splitView && !renderProps.hideLineNumbers && (
					<td
						...
						<pre className={renderProps.styles.lineNumber}>{additionalLineNumber}</pre>
					</td>
				)}
				<td
...
function RenderNode({
	...
	if (renderNodeWrapper) {
		return (
			<>
				{renderNodeWrapper(children, index)}
			</>
		);
	}
	return <>{children}</>;
				...
				<a
					onClick={() => {
						expandBlock(
							blockNumber,
							expandBlockIdsSet,
							(newState: Set<number>) => setExpandedBlockIdsSet(newState)
						);
					}}
					tabIndex={0}>
					{message}
...
function DiffViewer({
	...
	const [expandedBlockIdsSet, setExpandedBlockIdsSet] = React.useState<Set<number>>(new Set());
	...
	const props = {
		...
		const styles = React.useMemo(() => computeStyles(props.styles, useDarkTheme), [props.styles, useDarkTheme])
		const renderProps = React.useMemo<ReactDiffViewerRenderProps>(() => ({
			...props,
			styles,
		}), [props, styles]);
		const linesToRender = React.useMemo(() => getLinesToRender(renderProps, expandedBlockIdsSet).slice(0,100), [renderProps, expandedBlockIdsSet]);
